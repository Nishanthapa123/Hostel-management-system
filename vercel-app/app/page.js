...
export default function Home() {
  ...
  function login(event) {
    ...
    // Clear every form control immediately after submission so credentials are
    // not left visible while the login result is being processed.
    form.reset();
    setUsername('');
    setPassword('');
...
function AdminPortal({ view, setView, students, setStudents, rooms, setRooms, onLogout }) {
  ...
  const addStudent = (event) => { event.preventDefault(); setStudents([...students, newStudent]); setNewStudent({ id: '', name: '', phone: '', room: '', status: 'Paid' }); setShowForm(false); setView('students'); setToast('Student added successfully'); };
  const notify = (message) => { setToast(message); window.setTimeout(() => setToast(''), 2200); };
  return <main className="app-shell"><aside><div className="brand"><span className="mark small">⌂</span>HostelMS</div><nav>{nav.map((item) => <button key={item.id} className={view === item.id ? 'nav-active' : ''} onClick={() => setView(item.id)}>{item.icon} {item.label}</button>)}</nav><button className="logout" onClick={onLogout}>↪ Sign out</button></aside><section className="workspace"><header><div><span className="kicker blue">Admin workspace</span><h1>{nav.find((item) => item.id === view)?.label || 'Overview'}</h1><p className="muted">Manage your hostel operations from one workspace.</p></div><span className="profile-chip">SA</span></header>{view === 'overview' && <Overview students={students} rooms={rooms} filter={filter} setFilter={setFilter} setView={setView} notify={notify} />} {view === 'students' && <Students students={students} filter={filter} setFilter={setFilter} showForm={showForm} setShowForm={setShowForm} newStudent={newStudent} setNewStudent={setNewStudent} addStudent={addStudent} />} {view === 'rooms' && <Rooms rooms={rooms} setRooms={setRooms} notify={notify} />} {view === 'payments' && <Payments students={students} notify={notify} />} {view === 'requests' && <Requests notify={notify} />}{toast && <div className="toast-message">✓ {toast}</div>}</section></main>;
}

function Overview({ students, rooms, filter, setFilter, setView, notify }) { const visible = activities.filter((item) => item.join(' ').toLowerCase().includes(filter.toLowerCase())); return <><div className="stats"><Stat label="Total students" value={students.length} icon="♙" /><Stat label="Total rooms" value={rooms.length} icon="⌂" /><Stat label="Occupied rooms" value={rooms.filter((room) => room.occupied > 0).length} icon="▣" /><Stat label="Monthly revenue" value="₹24,000" icon="₹" /></div><div className="content-grid"><section className="panel wide"><div className="panel-head"><div><h2>Recent activity</h2><p className="muted">Live overview of hostel operations</p></div><label className="search">⌕<input value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Filter activity" /></label></div><div className="activity-list">{visible.map(([type, title, date]) => <div className="activity" key={title}><span className="activity-icon">{type === 'Payment' ? '₹' : type === 'Request' ? '!' : '♙'}</span><div><strong>{title}</strong><small>{type} · {date}</small></div><span className="arrow">→</span></div>)}</div></section><section className="panel"><div className="panel-head"><div><h2>Quick actions</h2><p className="muted">Common admin tasks</p></div></div><div className="quick-actions"><button onClick={() => { setView('students'); notify('Add a student from the Students page'); }}>＋ Add student</button><button onClick={() => { setView('payments'); notify('Payment records opened'); }}>＋ Record payment</button><button onClick={() => { setView('rooms'); notify('Room inventory opened'); }}>＋ Allocate room</button><button onClick={() => { setView('requests'); notify('Support requests opened'); }}>＋ Review requests</button></div></section></div></>; }
...
function Rooms({ rooms, setRooms, notify }) { return <section className="panel table-panel"><div className="panel-head"><div><h2>Room inventory</h2><p className="muted">Click a room to update occupancy</p></div></div><div className="room-grid">{rooms.map((room) => <button className="room-card room-button" key={room.number} onClick={() => { setRooms(rooms.map((item) => item.number === room.number ? { ...item, occupied: item.occupied < item.capacity ? item.occupied + 1 : 0 } : item)); notify(`Room ${room.number} occupancy updated`); }}><span className="room-number">{room.number}</span><strong>{room.type}</strong><small>{room.occupied}/{room.capacity} occupied</small><span className={room.occupied < room.capacity ? 'badge paid' : 'badge due'}>{room.occupied < room.capacity ? 'Available' : 'Full'}</span></button>)}</div></section>; }
